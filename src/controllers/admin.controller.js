...
const getAllPendingHotels = asyncHandler(async (req, res) => {
    const hotels = await Hotel.find({ approvalStatus: "pending" }).populate(
        "owner"
    );
    return res
        .status(200)
    ...
    return res
        ...
        .json(
            new ApiResponse(
                200,
                {},
                `Hotel ${req.body.approvalStatus} successfully`
            )
        );
});
    ...
    return res
        ...
        .json(
            new ApiResponse(
                200,
                {},
                "User rejected seller request successfully"
            )
        );
});
...
const updateIssue = asyncHandler(async (req, res) => {
    ...
    const user = await User.findById(issue.user).populate("issues");

    user.issues.map((prev) => prev._id === id ? { ...prev, status } : prev)
